...
public abstract class TunnelEventLimiter<T> {
	...
	Logger logger = LoggerService.getLogger(TunnelEventLimiter.class);
	...
	LocalCachedMapOptions<String, TunnelMessage<T>> localCacheOptions = LocalCachedMapOptions
			.<String, TunnelMessage<T>>defaults()
			.evictionPolicy(EvictionPolicy.NONE).cacheSize(5000).reconnectionStrategy(ReconnectionStrategy.NONE)
	...
	private RLocalCachedMap<String, TunnelMessage<T>> getCache() {
		if (cache == null) {
			cache = redisson.getLocalCachedMap(
					AppParam.APP_ENV.getValue() + TUNNE_LIMITER_MAP + this.eventClassName,
					localCacheOptions);
		}
		return cache;
	}
