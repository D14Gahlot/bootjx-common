...
public class WA360Client implements ChannelClient {
	...
	public OutboxMessage send(ChannelConfig channelConfig,
		...
		if (ArgUtil.is(outboxMessage.getTemplateExt())) {
			...
			if (outboxMessage.getRawMessageFormat() != null
					&& !outboxMessage.getRawMessageFormat().isEmpty()) {
			...
			for (PBVCard card : outboxMessage.getVccards()) {
				...
				if (ArgUtil.is(card.getUrls())) {
					...
					for (PBWebsite url : card.getUrls()) {
						urls.put("type", url.getType())
								.put("urls", url.getUrl()).map2list();
					}
			...
			if ("HEADER".equals(extTemplateComponentType)) {
				...
				if ("TEXT".equals(extTemplateComponentFormat)) {
					if (varMap.containsKey("header")) {
						...
						for (Map<String, Object> headerParameter : headerParametersTemp) {
							...
							String path2 = (String) headerParameter
									.get("path2");
							String defaultValue = (String) headerParameter
									.get("defaultValue");
							headerComponentReq.parameter("text",
									model.pathEntry(path).pathEntrySafe(path2)
				...
				if (varMap.containsKey("body")) {
					...
					for (Map<String, Object> bodyParameter : bodyParametersTemp) {
						...
						String defaultValue = (String) bodyParameter
								.get("defaultValue");
						bodyComponent.parameter("text", model.pathEntry(path)
								.pathEntrySafe(path2).asString(defaultValue));
					}
	...
	private String getMessageId(MapModel resp) {
		...
		String errorCode = resp.entry(OutBoundWrapperPaths.RESPONSE_ERRORS_CODE)
				.asString();
		if (ArgUtil.is(errorCode) || !ArgUtil.is(id)) {
			String errorTitle = resp
					.entry(OutBoundWrapperPaths.RESPONSE_ERRORS_TITLE)
					.asString();
			String errorDetails = resp
					.entry(OutBoundWrapperPaths.RESPONSE_ERRORS_DETAILS)
					.asString();
	...
	public MapModel mock(MapModel req) {
		try {
			MapModel resp = restService.ajax(
					"https://requestly.dev/api/mockv2/v1/messages?rq_uid=EaRk251ISeNyNfNTHKZ4ifiZbGv1")
					.header("user-agent",
	...
	public MapModel sendTemplateRaw(ChannelConfig channelConfig,
		...
		/*
		 * MapModel components = MapModel.createInstance(); List<Map<String,
		 * Object>> extTemplateComponents =
		 * extTemplate.keyEntry("components").asListOfMap();
		 * 
