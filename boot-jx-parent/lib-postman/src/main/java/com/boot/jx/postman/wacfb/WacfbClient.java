...
public class WacfbClient implements ChannelClient {
	...
	private String getMessageId(MapModel resp) {
		...
		if (ArgUtil.is(errorCode) || !ArgUtil.is(id)) {
			String errorTitle = resp
					.entry(OutBoundWrapperPaths.RESPONSE_ERROR_TITLE)
					.asString();
			String errorDetails = resp
					.entry(OutBoundWrapperPaths.RESPONSE_ERROR_DETAILS)
					.asString();

			ApiFieldError error = new ApiFieldError();
			error.code(errorCode);
			error.codeKey(errorTitle);
			error.setDescription(String.format("%s : %s / %s / %s ", id,
	 ...
	 * { try { MapModel resp =
	 * restService.ajax(WA360Constants.META_WA_CLOUD_URL)
	 ...
	 * .asMapModel(); return
	 * resp.path(OutBoundWrapperPaths.FETCH_CONTACTS_DETAILS) .asMapModel(); }
	 * catch (ApiHttpServerException e) { return
	 * MapModel.from(e.getResponse().getBody()).put(
	 * OutBoundWrapperPaths.RESPONSE_ERROR_CODE, e.getHttpStatus().value()); }
	 * catch (ApiHttpException e) { return
	 * MapModel.from(e.getResponse().getBody()); } }
	 */
