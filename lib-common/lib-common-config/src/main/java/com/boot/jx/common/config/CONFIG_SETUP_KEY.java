...
public enum CONFIG_SETUP_KEY implements EntryMeta {
		...
		// Agent Properties
		CHAT_TAG_ENABLED(
				new ConfigMeta("Chat Session Tags Enabled", "chat.tag.enabled").optionsOnOff().group(ConfigConstants.GROUP_AGENT)),
...
//		.inputType(OPTIONS_TYPE.COLOR_PALLETE).defaultValue(new ConfigMeta.ColorPalette()).group(GROUP_AGENT)),
		...
		POSTMAN_AGENT_TAB_ORG(new ConfigMeta("Agent can see Other Teams Chats", PROPERTIES.POSTMAN_AGENT_TAB_ORG)
				.hidden().deprecated().desc("Enables Org tab in Agent Panel").optionsOnOff().group(ConfigConstants.GROUP_AGENT)),

		POSTMAN_AGENT_TAB_LEVEL(new ConfigMeta("Agent can see Chats Assigned to", PROPERTIES.POSTMAN_AGENT_TAB_LEVEL)
				.desc("Enables Tabs in Agent Panel").optionValues("TEAM", "ORGANIZATION").group(ConfigConstants.GROUP_AGENT)),
		...
		POSTMAN_DEBUG_CONTACT(
				new ConfigMeta("Debugging is enabled for Contact", "postman.debug.contact").group(ConfigConstants.GROUP_DEV).hidden()),
